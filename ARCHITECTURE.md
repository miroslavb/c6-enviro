...
| Zigbee identity | **v0.1.9 recovery EUI `0x8efd49fffe1a3d8c`**, set little-endian after `esp_zb_init` and before `esp_zb_start` | Full-flash erase destroyed the old device key state while the coordinator retained the old EUI/link key. A unique local-admin EUI tests and avoids that stale trust-center record without coordinator-wide NVRAM surgery. |
...
| Interview | **5-minute MCU-awake, sleepy-radio window** after successful fresh steering or a cold boot with restored Zigbee NVRAM; first **60 s have no device-side bind/report traffic** and use **200 ms parent polls**; five-endpoint budget (EP1..EP5) | `rx_on_when_idle` remains false on every path. This v0.1.8 design remains a candidate until tested after device-side `JOINED`; the v0.1.8 field run never reached it because BDB steering failed upstream. |
