...
| `/root/c6-enviro` | Solar/Li-ion sleepy **End Device** | Same stack startup, NVS and +20 dBm TX; same five-endpoint interview budget as the successful v0.1.0 | `rx_on_when_idle=false` + 1 s polling normally; v0.1.12 permits continuous RX only for the bounded 5-minute cold-boot/BOOT interview window, then deep sleep. |
...
| Commissioning + interview | **200 ms sleepy parent polls begin before BDB steering** and continue through the first **60 s quiet ZDO phase** after `JOINED`; v0.1.12 also enables RX only for the enclosing **5-minute cold-boot/BOOT window**, then schedules it off; no bind/report traffic during the quiet phase; five endpoints EP1..EP5 | Live 2026-07-25 re-interviews still received announces but timed out on Active Endpoints after the 60 s polling phase. The bounded RX window keeps ZDO replies immediately reachable without changing the ZED role or timer-wake power profile. |
...
- `keep_alive` = 1 s: every wake polls the parent at least twice, so queued standard-cluster HA writes
...
- Factory-new boot instead steers (permit-join must be open), then holds the
  ...
  normal 1000 ms poll interval are restored before reporting; a power/reset/
  firmware-update cold boot and a short BOOT press use the same bounded window.
