...
- v0.1.10 recovery identity is `0x8efd49fffe1a3d8c`; hardware acceptance must use this IEEE and must first show device-side `JOINED`, not merely Z2M `device_joined`.
...
- Commissioning fast polling is bounded: 200 ms parent polls start before factory-new BDB steering, continue through the 60 s quiet interview phase, then restore the normal 1000 ms interval before reporting. This is not permission for always-on RX.
